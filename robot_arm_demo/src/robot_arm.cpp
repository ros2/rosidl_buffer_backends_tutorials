...
static inline torch::Tensor sdf_box(
  const torch::Tensor & px, const torch::Tensor & py,
...
static inline torch::Tensor sdf_box_rotated(
  const torch::Tensor & px, const torch::Tensor & py,
  float cx, float cy, float hx, float hy,
  float angle)
{
...
static inline torch::Tensor pencil_stroke_raw(
  const torch::Tensor & psdf, float lw,
...
static bool compute_bbox(
  float cx, float cy, float extent, float margin,
  int W, int H, int & y0, int & y1, int & x0, int & x1)
{
  x0 = std::max(0, static_cast<int>(std::floor(cx - extent - margin)));
  x1 = std::min(W, static_cast<int>(std::ceil(cx + extent + margin)));
  y0 = std::max(0, static_cast<int>(std::floor(cy - extent - margin)));
  y1 = std::min(H, static_cast<int>(std::ceil(cy + extent + margin)));
  return x0 < x1 && y0 < y1;
}

void RobotArmRenderer::draw_local_sketch_box(
...
void RobotArmRenderer::draw_local_sketch_box_rotated(
  torch::Tensor & frame, float cx, float cy,
...
void RobotArmRenderer::draw_local_sketch_circle(
  torch::Tensor & frame, float cx, float cy,
...
void RobotArmRenderer::draw_local_color_pencil_box(
  torch::Tensor & frame, float cx, float cy,
  float hx, float hy, float angle,
...
bool RobotArmRenderer::solve_ik(
  float tx, float ty, float & shoulder, float & elbow,
  float & wrist_world)
{
  ...
  wrist_world = static_cast<float>(M_PI) * 0.5f + 0.5f * horizontal_offset - 0.5f * height_factor;
...
void RobotArmRenderer::forward_kin(
  float shoulder, float elbow, float wrist_world,
  ...
  switch (state_) {
    ...
    case State::MoveToStack:
      if (do_move()) {
        ...
        int seed = current_pick_ * 13 + cubes_stacked_ * 7 + static_cast<int>(time_ * 10);
        float wobble = (rand_float(seed) - 0.5f) * cube_half_ * 1.0f;
...
void RobotArmRenderer::render_table(torch::Tensor & frame)
{
  ...
  int surf_y0 = std::max(0, static_cast<int>(std::floor(table_top - margin)));
  int surf_y1 = std::min(H_, static_cast<int>(std::ceil(table_top + margin)));
  int surf_x0 = std::max(0, static_cast<int>(std::floor(table_left - margin)));
  int surf_x1 = std::min(W_, static_cast<int>(std::ceil(table_right + margin)));
  if (surf_y0 < surf_y1 && surf_x0 < surf_x1) {
    ...
    auto x_mask = torch::sigmoid((lx - table_left) * 2.0f) *
      torch::sigmoid((table_right - lx) * 2.0f);
  ...
  for (float leg_x : {table_left + W_ * 0.05f, table_right - W_ * 0.05f}) {
    int lx0 = std::max(0, static_cast<int>(std::floor(leg_x - margin)));
    int lx1 = std::min(W_, static_cast<int>(std::ceil(leg_x + margin)));
    int ly0 = std::max(0, static_cast<int>(std::floor(table_top - margin)));
    int ly1 = std::min(H_, static_cast<int>(std::ceil(leg_bottom + margin)));
    ...
    auto y_mask = torch::sigmoid((rly - table_top) * 2.0f) *
      torch::sigmoid((leg_bottom - rly) * 2.0f);
...
void RobotArmRenderer::render_arm(torch::Tensor & frame)
{
  ...
  forward_kin(shoulder_angle_, elbow_angle_, wrist_angle_, elbow_x, elbow_y, wrist_x, wrist_y, ee_x,
    ee_y);
